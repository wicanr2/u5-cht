...
// 地上的物品、遊蕩的怪物。
//
// ⚠ **原本這裡寫「城鎮裡的 NPC 走的是另一套(`.NPC` + 排程),不在這裡」——
// 那句話錯了。** `sub_1E74` 會把「此刻在本層」的每一個 NPC 都鏡射進這張表
//(NPC 記錄的 +0xC 記住配到哪一槽,`sub_268` 移除時再放掉)。城鎮的寶箱、
// 地上的檀香木盒都是 `.NPC` 檔裡生物編號 < 0x40 的槽,靠這條鏡射才被 Get 看見。
// 見 `docs/re/36`。
//
...
const (
	...
	// ObjQuality 是數量 / 品質。
	//
	// ★ 原本記著「買馬時清零,但全檔沒有其他讀寫處」—— 已被推翻:
	// `sub_15A94` 把它當數量傳給 `sub_154BC`,而且**是哪一塊寶石碎片由它決定**
	// (`and eax, 3`);`sub_1E74` 幫寶箱填的也是這一格(0x1E)。
	ObjQuality = 5
	// ObjShipTick 是敵船的移動計數器(原版 `dword_3E470+3[槽*8]` = +7)。
	//
	// `sub_2D38` 每回合 `inc` 它,超過速度表的值就歸零並跳過那一回合
	// (`docs/re/84` §1)。⚠ 只有敵船用得到,其他種類這一格仍未解。
	ObjShipTick = 7
	// +6 仍未解,原樣保留不猜。
)
