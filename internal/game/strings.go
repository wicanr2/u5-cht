...
const (
	...
	MsgChestEmpty = "箱子是空的!"
	// Open(O)指令(原版 `sub_15374` / `sub_15108` / `sub_152B8`,`docs/re/75`)。
	//
	//	"It's open!\n" / "Too heavy!\n" / "Locked!\n" / "Opened!\n"
	//	"Nothing to open!\n" / "Can't!\n" / "Trapped!\n"
	//	"\nChest opened\n" / "Already Open!\n"
	...
	// 落空(原版 `sub_1F570`,`docs/re/74`)
	//
	// ★ 一支函式兩個出口,由 `byte_3E09F`(這一擊是幾圈的咒語;0 = 武器)決定:
	//
	//	byte_3E09F != 0  →  "Failed!"          ★ **不提是誰**
	//	byte_3E09F == 0  →  "<名字> missed!"
	//
	// 所以咒語落空在原版是一句不具名的「失敗了」,不是「某人擋下了咒語」——
	// 後者把因果講成了目標的功勞,而原版沒有那個意思。
	MsgFailed = "失敗!"
	MsgMissed = "揮空了。"
	...
	// 「Set Active Plr」的三句(原版 `sub_2BD40`)。
