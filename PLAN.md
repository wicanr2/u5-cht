...
| 層次 | 手段 |
|---|---|
| 單元 | `go test ./...`(無素材也全綠:合成資料 + 格式假設的自我一致性檢查) |
| 整合 | 設 `U5_GAMEDATA` / `U5_FMTOWNS` 後跑同一批測試,對原版素材驗證 |
| 視覺 | headless 截圖存 `tests/snapshots/`,逐畫面比對基準 |
| 行為 | 對 **DOSBox 原版**並排實測(`rulebook/65`:測試綠不算完成) |
| 可玩性 | game tester 跑正常玩家路徑(`retro-game-playtest`),無 debug hook |
