...
| 層 | 狀態 |
|---|---|
| 素材解碼 | 🔶 約 45%:tileset ✅、世界地圖 ✅、字型 ✅、對話 ✅(欄位未定)、明文文字 ✅;地底/場景/戰鬥圖/NPC/存檔/壓縮圖組 ⬜ |
| 顯示層 | 🔶 約 30%:地圖視窗 ✅、CJK 文字 ✅、HUD 骨架 ✅、戰鬥地圖可視化 ✅;地牢/戰鬥/選單畫面 ⬜ |
| **遊戲機制** | 🔶 **約 99%**:地表走動、進出 32 個地點的場景、梯子(101 段測過)、時鐘與 NPC 排程、對話與入隊、八種商店的完整交易、休息與恢復規則、地圖物件層、上下載具、NPC 逐格尋路、存檔讀寫、**完整的戰鬥回合**(遭遇生成 / 敏捷排程 / 敵人 AI / 命中與傷害 / 經驗值)、**魔法**(咒語表全解、15 個效果已實作)、**地牢**(格式全解、規則已實作,畫面仍是俯視)。**升級**(老人事件)。**投射物飛行**。**寶箱 / 力場 / 鎖門**。**月門**、**方向選單**、**風向與航行**。地牢第一人稱透視與其餘 6 個咒語效果仍未做 |
...
2. ~~場景地圖解碼~~ ✅(64 張 32×32);~~地點表~~ ✅(32 筆,`docs/re/03 §5`)。
   ...
   **剩下(依序)**:
   ...
   5. **魔法** —— 🔶 咒語表已全解(docs/re/17):48 個咒語名 + 圈數
      ...
      **42 個效果已實作**(光明 / 喚醒 / 解毒 / 治療 / 大治療 / 復活 /
      三個攻擊咒 / 魅惑 / 隱形 / 能量爆 / 造食物 / 定位 / 召喚 / 上下樓 /
      五個共用 `byte_3E08A` 的持續效果);其餘 22 個照原版扣資源後
      誠實回失敗,不假裝成功。
   ...
   6. **地牢** —— 🔶 格式已全解(docs/re/18):`DUNGEON.DAT` = 8 座 × 8 層 × 8×8,
      ...
      前兩座地牢共用同一批房間。
      已實作:進出、第一人稱朝向與前進後退轉向、爬梯、四種陷阱、
      房間觸發戰鬥、Uus Por / Des Por。
      ⚠ **畫面還是俯視的** —— 原版的第一人稱透視要先解 `DNG1-3.16`。
      **寶箱、力場、鎖著的門也已解並實作**(docs/re/21):地牢寶箱是格子(bit 0 = 陷阱,
      ...
      還缺:透視圖、噴泉的互動、地牢遊蕩怪、Doom 入口的三個門檻。
