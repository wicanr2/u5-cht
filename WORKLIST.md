...
| **遊戲機制** | 🔶 **約 96%**:地表走動、進出 32 個地點的場景、梯子(101 段測過)、時鐘與 NPC 排程、對話與入隊、八種商店的完整交易、休息與恢復規則、地圖物件層、上下載具、NPC 逐格尋路、存檔讀寫、**完整的戰鬥回合**(遭遇生成 / 敏捷排程 / 敵人 AI / 命中與傷害 / 經驗值)、**魔法**(咒語表全解、15 個效果已實作)、**地牢**(格式全解、規則已實作,畫面仍是俯視)。地牢第一人稱透視、其餘 33 個咒語效果、升級仍未做 |
...
2. ~~場景地圖解碼~~ ✅(64 張 32×32);~~地點表~~ ✅(32 筆,`docs/re/03 §5`)。
   ...
   **剩下(依序)**:
   ...
   5. **魔法** —— 🔶 咒語表已全解(docs/re/17):48 個咒語名 + 圈數
      ...
      **15 個效果已實作**(光明 / 喚醒 / 解毒 / 治療 / 大治療 / 復活 /
      三個攻擊咒 / 魅惑 / 時間停止 / 上下樓);其餘 33 個照原版扣資源後
      誠實回失敗,不假裝成功。An Tym 期間敵人不動、火把不燒。
