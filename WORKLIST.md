...
| 場景地圖 | 四檔各 16,384 B = 16 張 32×32(列寬由 `sub_86C` 證實) | ✅ **64 張全解出,且每張對應哪個地點/樓層已知**。畫面驗收:燈塔三層(底層有家具→二層塔身→頂層圓形燈室) |
| 地點類型 | `DATA.OVL` 0x2AB3 列 9 種;`sub_2D72C` 分派 12 種;**lighthouse 由四座燈塔的座標反推出是 tile 27** | ✅ tile 對應已知 |
| 進出場景(portal) | **`sub_2D72C` 依腳下 tile 分派**:16=hut、17=法典聖壇、18=keep、19=village、20=towne、21=castle、22=cave、23=mine、24=dungeon、25=八德聖壇、**27=lighthouse**、61=Blackthorn 宮殿、62=LB 城堡 | ✅ **對應規則全解**(`sub_5C8`):檔案=`SceneFiles[(編號-1)/8]`、索引=`byte_41033[編號]+樓層`、位移=索引×1024;含**多樓層**(燈塔 3 層、LB 城堡 5 層)。⬜ 進場景的狀態切換未實作 |
...
| **遊戲機制** | ⬜ **約 8%**:地表走動 + 原版通行規則 + 32 個地點(座標/名稱/場景對應/樓層數)。2.1–2.12 的其餘項目仍未做 |
...
2. ~~場景地圖解碼~~ ✅(64 張 32×32);~~地點表~~ ✅(32 筆,`docs/re/03 §5`)。
   ~~地點 → 場景地圖~~ ✅(`sub_5C8` 的三行算式,含多樓層)。
   **剩下:進場景的狀態切換** —— 把玩家從世界地圖移進 32×32 場景(進入點 `byte_3E0A6/A7` = 15,30)、
   場景內移動、離開時回到世界座標。這是引擎狀態機的工作,不是解碼。
