...
| **遊戲機制** | 🔶 **約 99%**:地表走動、進出 32 個地點的場景、梯子(101 段測過)、時鐘與 NPC 排程、對話與入隊、八種商店的完整交易、休息與恢復規則、地圖物件層、上下載具、NPC 逐格尋路、存檔讀寫、**完整的戰鬥回合**(遭遇生成 / 敏捷排程 / 敵人 AI / 命中與傷害 / 經驗值)、**魔法**(咒語表全解、**48 / 48 個效果已實作**)、**地牢**(格式全解、規則已實作,畫面仍是俯視)。**升級**(老人事件)。**投射物飛行**。**寶箱 / 力場 / 鎖門**。**月門**、**方向選單**、**風向與航行**。**全景(In Quas Wis)**。**地牢第一人稱透視**。戰鬥中的力場、視線遮蔽、走廊裡的物件(梯子/寶箱/噴泉,走 `ITEMS.16`)仍未做 |
   ...
   **剩下(依序)**:
   ...
   6. **地牢** —— 🔶 格式已全解(docs/re/18):`DUNGEON.DAT` = 8 座 × 8 層 × 8×8,
      ...
      **寶箱、力場、鎖著的門也已解並實作**(docs/re/21):地牢寶箱是格子(bit 0 = 陷阱,
      ...
      還缺:走廊裡的物件圖(`ITEMS.16`,表已 dump)、噴泉的互動與動畫、
      地牢遊蕩怪、Doom 入口的三個門檻、`0xC0` 的完整語意。
