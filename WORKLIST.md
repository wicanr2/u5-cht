...
| **視線遮蔽** | `sub_2E0E8` → `sub_2DDB0`;`sub_2E1D0`(19 擋 + 5 貼著才透);距離表 `byte_601F0` | 🔶 **已實作**(`docs/re/31`):flood fill 照抄、牆擋得住、Wis An Ylem 穿牆、戰鬥與石室不遮蔽。**夜間效果待對 DOSBox 實測**(§6)、**場景光源各自照明** `sub_2E21C` 未移植(§7) |
...
| **遊戲機制** | 🔶 **約 99%**:地表走動、進出 32 個地點的場景、梯子(101 段測過)、時鐘與 NPC 排程、對話與入隊、八種商店的完整交易、休息與恢復規則、地圖物件層、上下載具、NPC 逐格尋路、存檔讀寫、**完整的戰鬥回合**(遭遇生成 / 敏捷排程 / 敵人 AI / 命中與傷害 / 經驗值)、**魔法**(咒語表全解、**48 / 48 個效果已實作**)、**地牢**(格式全解、規則已實作,畫面仍是俯視)。**升級**(老人事件)。**投射物飛行**。**寶箱 / 力場 / 鎖門**。**月門**、**方向選單**、**風向與航行**。**全景(In Quas Wis)**。**地牢第一人稱透視(含走廊裡的梯子/寶箱/噴泉)**、**戰鬥中的力場咒語**、**Yell 指令**(收放帆 / 力量之言 / 召喚暗影君主)、**聖壇冥想與復原**、**任務旗標存讀檔**(封印 / 玷污 / 試煉 / 寶典 / 暗影君主 / 碎片)、**寶典**、**暗影君主的午夜遊走與碎片消滅**、**黑棘的審問**、**NPC 行為型別八種**(遊走 / 怕生 / 搭話 / 跟隨 / 醉步 / 敵對追擊)、**叫衛兵與逮捕**。**場景內與 NPC 的戰鬥**、**永久移除的存讀檔**、**結局兩條分支**、**四間 11×11 石室的畫面**(`MISCMAPS.DAT`)、**暗影君主盤據一座城**(現身 + 居民變敵對 / 逃跑)。**視線遮蔽**(牆擋得住、Wis An Ylem 穿牆;夜間效果待對原版、場景光源未移植)。石室裡的走位動畫、製作名單仍未做 |
