...
| 中文化 | 🔶 約 30%:管線 ✅、UI 字串 ✅、**譯名層 ✅**(`internal/i18n`:48 生物 + 48 裝備 + 8 藥草 + 40 地點 + 135 個 NPC 名,**與 u6-cht 對齊**,政策見 `docs/manual/術語對照.md`);**開場動畫已實作**(21 頁,頁表 dump 自原版 `sub_584B`)、**114 筆明文全翻**(`internal/i18n/text.go`:開場故事 20、吉普賽問答 30、業報 6、系統訊息 47、結局 11);`.TLK` 對白本文未翻、`SHOPPE` 卡在 token |
