...
| 場景地圖 | `TOWNE/CASTLE/KEEP/DWELLING.DAT` 各 16,384 B;`DATA.OVL` 0x2636 列出這四個檔 | ⬜ 推測 16 個 32×32 地圖/檔,待驗 |
| 地點類型 9 種 | `DATA.OVL` 0x2AB3:`keep village towne castle cave mine dungeon ruins lighthouse` | ⬜ |
| 進出場景(portal) | 同上 + `TOWN.OVL` | ⬜ |
| 地形通行規則 | `MAINOUT.OVL`/`OUTSUBS.OVL`;tile 語意表待取 | ⬜ **目前可穿牆過海** |
| 世界環繞(wrap) | 已實作並測試 | ✅ |
...
| **遊戲機制** | ⬜ **約 2%**:只有「在地表上走動」而且**沒有通行規則**(可穿牆過海)。2.1–2.12 的其餘項目全部未做 |
...
1. **tile 語意表**(哪個索引是水/草/山/城鎮入口)→ 通行規則。沒有它,現在可以穿牆過海。
   來源:P3 反編譯碼,或從 `DATA.OVL` 找 tile 屬性表(疑為 `PROPORT.PCS` 802 B —— 名稱像 "properties")。
