...
| **三位暗影君主** | `sub_17A14` / `off_55DEC`;`DATA.OVL` 0x484F:`doom of the Shadowlord **Faulinei / Astaroth / Nosfentor**` | 🔶 **召喚已實作**(在三團聖火所在喊名字,`docs/re/26` §5);遊走(`sub_29304`)、玷污聖壇(`sub_C318`)、消滅(`sub_1A38C`)未接 |
| **三個邪惡碎片** | `sub_1A38C`;`DATA.OVL` 0x47C3:`evil Shard of **Falsehood… / Hatred… / Cowardice…**` | ⬜ U5 核心:摧毀碎片才能滅暗影君主 |
...
| 指令系統 | `CMDS.OVL` 7,440 B | 🔶 已接:E 進入 / K 攀爬 / T 交談 / B・X 上下載具 / C 施法 / I 火把 / **Y 喊**;Look・Get・Open・Push 等未接 |
...
| **遊戲機制** | 🔶 **約 99%**:地表走動、進出 32 個地點的場景、梯子(101 段測過)、時鐘與 NPC 排程、對話與入隊、八種商店的完整交易、休息與恢復規則、地圖物件層、上下載具、NPC 逐格尋路、存檔讀寫、**完整的戰鬥回合**(遭遇生成 / 敏捷排程 / 敵人 AI / 命中與傷害 / 經驗值)、**魔法**(咒語表全解、**48 / 48 個效果已實作**)、**地牢**(格式全解、規則已實作,畫面仍是俯視)。**升級**(老人事件)。**投射物飛行**。**寶箱 / 力場 / 鎖門**。**月門**、**方向選單**、**風向與航行**。**全景(In Quas Wis)**。**地牢第一人稱透視(含走廊裡的梯子/寶箱/噴泉)**、**戰鬥中的力場咒語**、**Yell 指令**(收放帆 / 力量之言 / 召喚暗影君主)、**聖壇冥想與復原**、**任務旗標存讀檔**(封印 / 玷污 / 試煉 / 寶典 / 暗影君主)、**寶典**。視線遮蔽、聖壇石室的畫面、暗影君主的遊走與消滅仍未做 |
| 中文化 | 🔶 約 30%:管線 ✅、UI 字串 ✅、**譯名層 ✅**(`internal/i18n`:48 生物 + 48 裝備 + 8 藥草 + 40 地點 + 135 個 NPC 名,**與 u6-cht 對齊**,政策見 `docs/manual/術語對照.md`);**聖壇與冥想已實作**(八座座標、真言、前綴比對、試煉旗標、三圍與業報獎賞、獻金)、**力量之言與暗影君主已實作**、**寶典已實作**(八段箴言 + 四段符文全譯)、**開場動畫已實作**(21 頁,頁表 dump 自原版 `sub_584B`)、**114 筆明文全翻**(`internal/i18n/text.go`:開場故事 20、吉普賽問答 30、業報 6、系統訊息 47、結局 11);`.TLK` 對白本文未翻、`SHOPPE` 卡在 token |
