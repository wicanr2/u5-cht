...
| 進出場景(portal) | **`sub_2D72C` 依腳下 tile 分派**:16=hut、17=法典聖壇、18=keep、19=village、20=towne、21=castle、22=cave、23=mine、24=dungeon、25=八德聖壇、**27=lighthouse**、61=Blackthorn 宮殿、62=LB 城堡 | 🔶 入口 tile ✅;**地點表 ✅**(32 筆座標+名稱,26/32 交叉驗證命中);⬜ 實際載入場景的動作未做 |
...
| **遊戲機制** | ⬜ **約 6%**:地表走動 + 原版通行規則(阻擋 195/512)+ **32 個地點的座標與名稱**(踏上時顯示中文地名)。2.1–2.12 的其餘項目仍未做 |
...
2. ~~場景地圖解碼~~ ✅(64 張 32×32);~~地點表~~ ✅(32 筆,`docs/re/03 §5`)。
   **剩下:地點索引 → 哪一張場景地圖**。地點表給了座標與名稱,但沒說 BRITAIN 對應
   `TOWNE.DAT` 的第幾張。下一步讀 `sub_10928` 後半(`byte_3E0A3 = i + 1` 那裡)與 `sub_2CB3C`。
