...
| **遊戲機制** | 🔶 **約 68%**:地表走動、進出 32 個地點的場景、場景內移動與邊界離開、梯子(101 段測過)、時鐘與 NPC 排程、對話與入隊、**八種商店的完整交易**、休息與恢復規則。戰鬥、魔法、地牢、載具、地圖物件、存檔寫回仍未做 |
...
2. ~~場景地圖解碼~~ ✅(64 張 32×32);~~地點表~~ ✅(32 筆,`docs/re/03 §5`)。
   ...
   **剩下(依序)**:
   ...
   🔶 **商店只剩兩處**,兩處都是缺前置系統不是缺規則:買馬買船後的坐騎物件
      要地圖物件層;酒館的打聽消息要八德與地牢的知識表(`off_56E9C` 16 個關鍵字)。
   1. **NPC 每回合移動** —— 現在是直接放到排程位置;原版 `sub_9690` 會逐步尋路。
   2. **叫衛兵**(opcode 0x8B → `sub_C10`)—— 需要戰鬥系統當落點。
   3. 戰鬥、魔法、地牢、載具、存檔寫回。
