...
| 商店 | 8 種店、47 家;`byte_4185C`/`off_4145C`/`off_4165C` 三張稀疏表 + `SHOPPE.DAT` 問候語 | 🔶 **目錄 + 價目表 + 五種店的交易已解**(docs/re/08、10):店名店主與 15 張價目表都從 DATA.OVL 讀;七個佔位符 `#$%&*@^` 全解;買價與賣價兩條公式;武具店買賣、藥草、公會、治療所、馬廄可跑。⬜ 酒館 / 造船廠 / 旅店 |
...
| **遊戲機制** | 🔶 **約 63%**:地表走動、進出 32 個地點的場景、場景內移動與邊界離開、梯子(101 段測過)、時鐘與 NPC 排程、對話與入隊、五種商店的交易。戰鬥、魔法、地牢、載具、存檔寫回仍未做 |
...
2. ~~場景地圖解碼~~ ✅(64 張 32×32);~~地點表~~ ✅(32 筆,`docs/re/03 §5`)。
   ...
   **剩下(依序)**:
   ...
   🔶 **商店剩三種**:酒館(點餐 / 八卦)、造船廠(價已解,缺海面載具生成)、
      旅店(價已解,缺休息 / 寄放同伴)。馬廄買馬後的地圖物件也要等物件層。
