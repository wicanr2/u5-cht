...
| **結局的觸發** | `sub_161E4`(哨兵 `byte_3E0B0 = 'M'`)→ `sub_42CC` / `sub_163B0` 兩處查它 → `sub_135FC` | ✅ **條件解出來了**(`docs/re/34`):活著的單位站在**戰場第 2 列**、**正北**那一格是城堡外牆或城門(0x3C..0x3F)就被「吸收」,戰鬥收尾播結局。<br>更正兩處讀錯:`byte_3E0B0` 是**撤離方向**(`sub_15E20`「All must use the same exit!」),'M' 只是塞進去的哨兵;`byte_3F854 = byte_3F844 + 16` 是戰場暫存(列距 16)的**第 1 列**,不是腳下。城堡地形在世界地圖上只有七格,全圍著兩座城堡入口。<br>⬜ 引擎的戰場地形目前只來自 `.CBT` 固定地圖(一格城堡都沒有),要真的觸發得到還缺 `sub_295AC` 那條「野外用周遭地形當場組戰場」 |
