...
| **遊戲機制** | 🔶 **約 74%**:地表走動、進出 32 個地點的場景、梯子(101 段測過)、時鐘與 NPC 排程、對話與入隊、八種商店的完整交易、休息與恢復規則、地圖物件層、**上下載具**。戰鬥、魔法、地牢、NPC 尋路、存檔寫回仍未做 |
...
2. ~~場景地圖解碼~~ ✅(64 張 32×32);~~地點表~~ ✅(32 筆,`docs/re/03 §5`)。
   ...
   **剩下(依序)**:
   ...
   1. **NPC 每回合移動** —— 🔶 結構已逆完並寫成 docs/re/12(執行期狀態 16 B、
      八種行為模式、32 格環狀佇列的有界 BFS、方向順序取決於「從哪來」、
      重試節流 200 次 + 1/3 機率)。**尚未實作**:還缺 `sub_9358`、
      回溯成 (步數,方向) 的那段、`byte_400F4` 樓梯表。
      在補齊前維持瞬移 —— 走法不同會讓 NPC 站在不同格子,那是玩得到的差異,
      所以不寫近似版。
