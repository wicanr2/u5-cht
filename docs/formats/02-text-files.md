...
> 狀態:**明文檔已可解析**;`SHOPPE.DAT` 的詞典 token 映射未定;`LOOK2`/`SIGNS` 另案。
...
| 檔案 | 觀察 | 狀態 |
|---|---|---|
| `LOOK2.DAT` 3,622 B | 218 個 NUL + 大量 `0x01–0x1F` 控制碼(0x04×52、0x06×65、0x0C×106…) | 結構化格式,未解 |
| `SIGNS.DAT` 8,364 B | 開頭是 u16 offset 表;內容是用字元畫的框線(`8lllllmllmlllll9`)+ 地名(`NOR[@BRITAIN`) | 同 u4-cht 的「美術內嵌字母」問題,先評估再決定譯不譯 |
