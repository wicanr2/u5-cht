...
**觸發是瀑布**:兩處都判 `tile & 0xFC == 0xD4`,而 `look#212..215` 四格都是「瀑布」。
...
1. **傷害是固定 1 點**,不是骰的 —— `sub_2B724` 只決定「有沒有受傷」。
   瀑布不是陷阱,是交通工具。所以敏捷 31 以上的人永遠不會受傷(門檻最大 30)。
2. **通往幽冥界的座標寫死在程式裡**(54, 138),世界上只有那一個瀑布下得去;
   其餘瀑布只會讓你掉一次、扣一點血,原地不動。
3. 那個畫面效果**有還原**載具碼(`mov eax, esi; mov byte_3E08C, al`),
   與 `sub_22F0` 溺水那一段的「設 0 之後**不**還原」正好成對 ——
   兩處互相印證:那個 0 真的是「不畫載具圖」,不是某種載具。

落地在 `game/troll.go` 的 `fallDownTheWaterfall`,四條測試:
傷害正好 1 點、敏捷 99 永遠不受傷、**相鄰座標都下不去**(擋「把條件寫成範圍」)、
瀑布是 0xD4..0xD7 四格而 0xD3 / 0xD8 不算。
