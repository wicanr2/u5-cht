...
### ★ 色號 6 是暗黃(0xAAAA00),不是棕(0xAA5500)

EGA 調色盤值的位元佈局是 `r g b R G B`(bit 5/4/3 次級、bit 2/1/0 主級),
每通道兩位元四級:`00`→0x00、`01`→0x55、`10`→0xAA、`11`→0xFF。

- **0x06** = `000110` → 紅與綠皆主級 → **(AA, AA, 00) 暗黃**
- 0x14 = `010100` → 紅主級 + 綠次級 → (AA, 55, 00) 棕 ← **慣例值,原版沒用**

而色號 6 佔整份 tileset 的 **7.70%** —— 黑(56.96%)與白(6.07%)之後**第三多**。
沙漠、木地板、門、桌椅、柵欄、寶箱全是它。所以這一格錯了,每一張畫面都偏色。

⇒ `EGAPalette` 從「抄慣例」改成**從原版的表推導**:
`LoadEGAPalette(dir)` 直接讀那 17 B,而 `TestEGAPaletteMatchesTheTableInDataOVL`
要求常數與表**逐格相符**。哪天有人把色號 6「修回」棕色,那條測試會紅。
