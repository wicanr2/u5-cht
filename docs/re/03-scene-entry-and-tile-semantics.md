...
⇒ **`sub_3181C` 的參數直接就是場景索引**(不是先前推測的「場景組別」)。
所以 `sub_2D72C` 傳的 6 / 7 / 8 / 11 就是場景編號本身,範圍 0–14(`<= 0xE`)。

⚠ 但這還不是完整的地點表:hut 與 village 都傳 8,towne 走另一條路徑 ——
**同類型的多個城鎮怎麼區分,仍未解**。真正決定「哪一個城鎮」的應該是玩家的世界座標,
存在別的變數裡(`dword_65338` 是候選,由 `sub_31CCC` 讀取)。
