...
3. ★ **餓的判定不看用餐時刻,但仍然每小時只跑一次。** 兩件事要分開看:
   挨餓那條路**繞過**「是不是 6/12/18 點」的檢查(所以任何小時都會餓),
   但它的結尾是 `jmp short loc_2A5C0`,而 `loc_2A5C0` 就是
   `byte_3E090 = byte_3E08F` —— 三條路的匯流點。
   ⇒ **斷糧的隊伍每小時掉一次血**,而不是每走一步掉一次。

✅ `byte_3E09B` 是**每回合 +1**(上限 255)的計數器,不是進餐計數器 ——
它的 `+1` 在 `loc_2A5CA`,而「這小時已結算」那條路直接跳到那裡(`docs/re/99` §5b)。
用途是施捨業報的節流。
...
- ~~`byte_3E09B` 的用途~~ ✅ 已解:回合計數器 + 施捨業報節流(`docs/re/99`)。
