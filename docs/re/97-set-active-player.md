...
## 3b. ⬜ 規格已完整的一項:多人可選時原版會**問**

`sub_E19C` 的第三條路在「2 人以上能動」時會問,而選單是 `sub_2A7F4`
(經 `sub_2A974` 包一層)。兩支都讀完了,規格如下:

```
印 "Player: "
sel = sub_2A7F4(允許Enter)
  ; 游標式選單,標題 "Select:"
  ;   '1'..'6'      → 直接跳到那一位(< 隊伍人數 才採用)
  ;   鍵碼 1 / 3(↑ ←)→ 游標 −1(下限 0,`sub_2BC18`)
  ;   鍵碼 2 / 4(↓ →)→ 游標 +1(上限 人數−1,`sub_2BBDC`)
  ;   '0'           → sel = −1
  ;   空白 / ESC     → 結束
  ;   Enter         → 只有 arg_0 非 0 時才 sel = −2 並結束
  ; ⚠ '1'..'6' 與方向鍵**不衝突** —— 方向鍵是鍵碼 1..4,在 '0'(0x30)之下
if (sel < 0)                      → −1 印 "None!\n"
if (狀態 != 'G' && 狀態 != 'P')    → ★ 印 "Disabled!\n\n" 並**重問**
印 隊員[sel].名字
```

★ 值得記的是 **`Disabled!`**:選單**讓你把游標停在死人身上**,按下去才告訴你不行,
然後回到選單。不是「游標跳過死人」——兩種做法玩家的手感完全不同。

⬜ **為什麼還沒接**:引擎的 `pickCharacter` 是**同步**的(回傳 int),
而選單必須是非同步的(`PromptPick` + callback)。要接就得把
**11 個呼叫端**(`dungeonsearch` ×2、`dungeonlook`、`commands` ×2、`search`、
`look` ×3、`chest` ×2)全部改成 callback 形式 —— 那是一次橫向重構,
風險在「每個流程的後續步驟都要跟著搬」。

★ 而 `SetActivePlayer`(§1)已經把**實務上的差別縮到很小**:玩家可以先指定,
指定過就不會問 —— 那正是原版給的同一個逃生門。目前的落差只剩
「沒指定 + 多人可選」時原版問、引擎取最後一位。

⇒ 保留成**規格完整的 ⬜**:接的時候不必再逆向,照上面那段寫即可。
