...
## 6. 補記:被盤據的城裡會發生什麼(`sub_48C` + `sub_15C4`)

§5 把 `byte_3E16A` 的用處列為未知。它在進場景的 `sub_1678` 裡串起兩支:

```c
sub_48C();                      // byte_3E16A = 盤據這裡的是第幾位;並讓牠現身
if (byte_3E0A3 == 0x1D)         // 石門 —— 三位的出身地
    for (i = 2; i >= 0; i--)
        if (byte_3E0D8[i] < 0x80) sub_1638(i);      // 「空氣中瀰漫著…」
else if (byte_3E16A != 0xFF) {
    sub_1638(byte_3E16A);
    sub_15C4();                 // ★ 城裡的居民變了
}
```
...
```c
byte_3E16A = 0xFF;
if (byte_3E0A7 != 4)                                   // ★ 玩家 Y 剛好是 4 就整段跳過
    for (i = 0; i < 3; i++)
        if (byte_3E0D8[i] == byte_3E0A3) { byte_3E16A = i; break; }
if (byte_3E16A == 0xFF) return;
if (場上已經有 tile 0xFC) return;
…在 (0x0F, byte_3EF1B[地點]) 生一隻,AI = 6(敵對)…
```

★ `byte_3EF1B` = `[0, 4, 9, 15, 8, 17, 10, 11, 10, 0, 0, …]` ——
**只有索引 1..8 非零**,而那正是 `sub_29304` 分派的範圍(`random(1,8)`)。
兩份獨立資料對上,不必另找 oracle。

⚠ 「玩家 Y 剛好是 4 就跳過」看不出理由,但照抄 ——
少了它,在某些城的某些落點會多出一隻。

### 6.2 居民變了(`sub_15C4`)

| `byte_3E16A` | 效果 | 對話號碼改成 |
|---|---|---|
| 0 FAULINEI 虛偽 | **什麼都不做**(它的手段在別處) | — |
| 1 ASTAROTH 憎恨 | 居民**變敵對**撲上來(`sub_B44`) | 0xFE |
| 2 NOSFENTOR 怯懦 | 居民**四散奔逃**(`sub_B98`) | 0xFD |
...
```asm
for (esi = 0; esi < 4; esi++)
    if (byte_3E57C[npc*16 + esi]) ebx = 1;      // 有作息的居民
loc_1594:
movzx eax, byte_3EDB0[esi]                      ; ★ esi,不是 edi
cmp   eax, 40h ; jl  → ebx = 0
cmp   eax, 74h ; jge → ebx = 0
random(0,1) != 0 → ebx = 0                      ; 一半機率
```

`esi` 在迴圈結束時是 **4**,不是傳進來的 npc(`edi`)。所以
**生物編號那一關檢查的永遠是第 4 號槽的居民**,與正在判的那個無關。

照抄 —— 與末日位元共用儲存那個同理。差別看得出來:4 號槽剛好是隻動物的城,
整城就沒有人會被影響。自己「修好」它會讓行為與原版不同。

### 6.4 關於末日位元與地點 29 那個共用儲存

`docs/re/29` §7.3 把它記成「疑似原版 bug」。這一輪多出一個佐證:
**石門(地點 29)本來就是暗影君主相關的地點** —— 進去會為每一位還沒被消滅的
印一次「空氣中瀰漫著……」。所以「消滅一位就把石門的某個 NPC 槽標成清掉」
也可能是刻意的。兩種解讀都成立,引擎照抄同一塊儲存,不猜。
