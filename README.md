...
這一整條主線都跑得起來。中文化方面:介面、譯名、五個明文檔與**商店對白 194 段**已完成,
**NPC 對白本文才翻了 21 / 1,853 段**(見下方「還沒到的地方」)。
...
- **NPC 對白本文才剛起步(21 / 1,853 段)**。名字、物品、地名、怪物名、
  五個明文檔的 114 筆,以及**商店對白 194 段全數**都翻好了;`.TLK` 裡的
  NPC 對白本文絕大多數還是英文。管線已就位(`internal/i18n/talk.go`,
  key = `檔名#對話id#欄位`),工作清單用 `u5dump talkwork` 產(**英日對照**,
  日文靠索引表的 id 欄對齊,只當語意佐證不當譯名來源)。
