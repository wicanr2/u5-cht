...
兩種版面都固定保留原版藍白框線與底部命令提示 footer 空間；按 `F2` 只切換右欄排版。
現代版面仍保留分組留白，原版版面則維持原版欄位順序；兩者都保留日／月指示，以及
remake 擴充的繁中訊息與狀態內容。
...
| ![現代版面（固定古典框線）](docs/screenshots/25-ui-modern.png) | ![原版版面](docs/screenshots/26-ui-original.png) |
| **現代版面(預設)** —— 藍白框線 + 分組留白 + 分隔線,保留日／月圖指示與 remake 擴充訊息 | **原版版面** —— 藍白框線 + 欄位組成與順序照原版右欄,底部命令提示不被框線遮住。`F2` 只切換排版 |
