...
《創世紀 IV》裡,汝走完八德的修行,成為聖者 —— 不列顛尼亞公認的道德典範。
第五代開場,那片土地已經不認得汝了。

不列顛王親自下到地底世界探勘,沒有回來。王位空著,攝政的是黑棘勳爵,
而在他耳邊說話的是三名影主:虛偽、憎恨、怯懦,各自從一塊碎片裡生出來的東西。
黑棘做的事說起來很簡單 —— 他把八德寫成了法律。

於是慈悲不再是選擇。街上站著攔路的衛兵,開口就要「給黑棘的貢金」:
一般城鎮是每個活著的隊員十枚金幣,米諾克要汝一半的家財。付不出來就當場逮捕 ——
**願意付而身上沒錢的,一樣抓**。汝真的被抓到黑棘面前時,他問的是聖壇的真言:
說出來,那座聖壇當場被玷污;一路不說,鍘刀就落在汝同伴的頸上。
招與不招都要死人 —— 這就是「以美德治國」長出來的樣子。

要結束這件事,汝得走遍八座聖壇問出力量之言,到地牢深處取回三塊碎片,
再站到**對應**的那一團聖火前把它舉起來 —— 配錯了火,那位影主打不掉。
最後下到地底世界把王帶回來。而汝一開始什麼都沒有:同伴散落各處,
連一把像樣的劍都得自己去店裡買。

1990 年代的台灣玩家是靠《軟體世界》雜誌的說明書翻譯讀懂這個故事的:遊戲本身
一個中文字都沒有。這個 repo 就是把那件事補上 —— 從反組譯原版執行檔開始,
用 Go 從零把引擎重寫一遍,然後讓不列顛尼亞的每一個人都說中文。
...
![古典版 GUI（命令提示 footer 不被框線遮住）](docs/screenshots/26-ui-original.png)

古典版面以原版藍白框線與欄位順序為基礎；按 `F2` 可與現代版面即時切換。
框線在底部命令提示 footer 前收邊，提示文字保留獨立空間；兩種版面都保留日／月指示，
以及 remake 擴充的繁中訊息與狀態內容。
...
| ![現代版面](docs/screenshots/25-ui-modern.png) | ![原版版面](docs/screenshots/26-ui-original.png) |
| **現代版面(預設)** —— 分組留白 + 分隔線,保留日／月圖指示與 remake 擴充訊息 | **原版版面** —— 原版藍白框線 + 欄位組成與順序照原版右欄,底部命令提示不再被框線遮住。`F2` 隨時切換 |
...
《創世紀 IV》問的是「你願不願意成為有德之人」;第五代把同一套美德交到暴君手上,
改問**當美德變成統治工具時,你怎麼辦**(故事見開頭)。
...
| **`F2`** | **切換版面** —— 現代版面(預設)⇆ 原版藍白框線；兩者都保留日／月圖指示 |
| `F3` | 除錯欄位(座標與地形碼;原版沒有這兩欄,預設關) |
| **`F5`** | **即時存檔** —— 原地存,不離開遊戲 |
| **`F6`** | **讀回存檔** |
...
打三平台交付包時，公開版不含原版資料；macOS 另產 arm64 + x86_64 universal：

```bash
tools/package.sh v0.2.1 --full                 # Linux／Windows 完整版，產物在 dist-local/
tools/build-mac-osxcross.sh v0.2.1 --full      # macOS universal 完整版，產物在 dist-local/
tools/package.sh v0.2.1 --patch                # Linux／Windows patch，產物在 dist/
tools/build-mac-osxcross.sh v0.2.1 --patch     # macOS universal patch，產物在 dist/
```

完整版含本機自備的原版資料、中文字庫與音樂，只能留在 `dist-local/`，不得上傳或散布。
patch 只含新執行檔、版本資訊、授權與 SHA-256 清單，解壓到對應完整版目錄覆蓋即可。

公開交付包（不加 `--full`）仍**不含原版資料,也不含烘好的中文字庫** —— 前者是玩家自備的合法副本,
...
- 上面的畫面截圖與 `docs/promo/` 的推廣短片含有原版美術,僅為說明本專案的呈現效果;
  權利人要求即撤除。**推廣短片入庫的那一份沒有配樂** —— 有配樂的版本用的是原版光碟
  音軌,那與原版資料同樣不散布,只留在本機。
