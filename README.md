...
| ![衛兵盤查](docs/screenshots/17-guard.png) | ![製作名單](docs/screenshots/19-credits.png) |
| **衛兵索取貢金** —— 全遊戲只有十三個攔路衛兵(對話號碼 0xFF),分布在五座城與黑棘的宮殿。付不出來就當場逮捕 | **真結局的頒獎狀** —— 中間那兩行原版是用符文字型畫的,維持英文;起算日 139 年 4 月 5 日與 `INIT.GAM` 對得上 |
...
- **Get 指令接好了 —— 碎片與檀香木盒總算撿得起來**。這是三塊寶石碎片、
  ...
  **三塊碎片與寶珠擺在哪裡**也找出來了:它們不在任何資料檔裡
  ...
  只剩檀香木盒擺在哪裡還沒找。
- **觸發結局的條件追到一半**。結局那一幕本身完整了(含「沒帶盒子就沒有真結局」
  那條分支),但「什麼時候自動觸發」還差兩個語意 ——
  鏈已經追到底(`sub_135FC` 只有兩個呼叫點,都守著 `byte_3E0B0 == 'M'`;
  全檔只有 `sub_161E4` 設它,條件是「某個戰場單位站在 Y=2、而某張表在它 X 的
  那一格是 0x3C..0x3F」),只剩那張表的身分沒定。
  過程與**三處掃描 + 正對照**寫在 [`docs/re/34`](docs/re/34-ending-trigger.md)
  —— 正對照證明 0x3C..0x3F 在地圖資料裡幾乎不存在(命中幾乎全是文字檔裡的
  ASCII `<=>?`),所以那不是地形查表。**沒有猜著實作。**
  目前只能用 `u5dump` 的腳本播。
