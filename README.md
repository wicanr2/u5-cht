...
![古典版 GUI 界面](docs/screenshots/26-ui-original.png)

古典版面以原版藍白框線與欄位順序為基礎；按 `F2` 可與現代版面即時切換。
兩種版面都保留日／月指示，以及 remake 擴充的繁中訊息與狀態內容。
...
| **現代版面(預設)** —— 分組留白 + 分隔線,保留日／月圖指示與 remake 擴充訊息 | **原版版面** —— 原版藍白框線 + 欄位組成與順序照原版右欄,一行接一行。`F2` 隨時切換 |
