...
- **什麼事會惹毛衛兵還沒接**。逮捕本身完整了(NPC 會走過來、問汝束手就擒否、
  紫杉城的牢房),但「偷竊 / 攻擊平民 → 衛兵敵對」那個判定還沒對上原版的哪一支;
  目前只有對話裡的「叫衛兵」進得去。
- **石室裡不會走動**。四間 11×11 石室(聖壇 / 寶典 / 審問 / 王座廳)畫出來了,
  但原版是一格一格走進去、走出來的(`sub_134CC`);這裡直接站在走完的那一格。
- **音樂與音效還沒接**(P6)。
- **視線遮蔽做好了,夜間效果還沒對過原版**。牆與山會擋住後面的東西,站在
  房間裡看不到隔壁房間,Wis An Ylem 也真的能穿牆了(`sub_2DDB0` 的 flood fill
  照抄,含那個「牆從亮的一側看得到、暗的一側看不到」的重判規則)。
  但讀完組語得到一個與直覺相反的結論:**半徑不參與傳播,所以在資料完整的
  場景裡夜晚與白天的可見範圍相同** —— 這一條還沒對 DOSBox 實測,
  理由與三種可能寫在 [`docs/re/31`](docs/re/31-line-of-sight.md) §6。
  另外**場景光源各自的照明**(`sub_2E21C`:牆上的火把照亮房間)還沒移植。
