...
- **可用價值**:**16 首 `.XMI`**(`U5THEME` `BRITLAND` `STONES` `WRLDBLW` `RULEBRIT` `MONARCH` `GREYSON` `LADYNAN` `ENGGMNT` `REUNION` `HALLS` `FANFARE` `HORNPIPE` `AMIGA` `BLCKTHRN` `trntlla`)+ AIL/MIDPAK 驅動。⚠ 另有 `setm.xmi`,裡面裝**五首**,是驅動的測試檔不是遊戲音樂。⚠⚠ `trntlla.xmi` **檔名小寫**,`*.XMI` 這種大小寫敏感的 glob 會漏掉它。→ ✅ **已離線轉 ogg**:XMI→MID(`tools/xmi2mid.py`)→ Roland MT-32(munt + 使用者提供的 ROM,`tools/mt32_render.sh`),不跑模擬器。曲號對應靠旋律比對解出(`docs/re/98`)。
...
### 3.0 [HARD] 素材與遊戲機制一律還原原版(使用者 Goal,2026-08-07)

**機制與素材同一條鐵則:忠於原版,不自創、不自行平衡。**

- 規則、數值、公式(戰鬥傷害、法術消耗、業報增減、NPC 排程、風向影響…)一律以原版為準。
  來源優先序:**反編譯碼 > 原版資料表 > 《軟體世界》手冊 > 社群文件**。
- **沒有證據就不要實作**:憑對 U5 的印象寫規則,等於自創遊戲。標 TODO 留白比猜著寫好。
- 逐子系統的盤點與現況見 **`WORKLIST.md`**(依 24 個 `.OVL` + `DATA.OVL` 的 43 個明文表建立)。
- 宣稱任一機制「完成」前,對 DOSBox 原版實測比對(`rulebook/65`)。
...
| 音樂 | FM Towns **CDDA 兩軌**、15 首 `.EUP`、upgrade 的 **16 首 `.XMI`**、PC-98 `UL*.BIN` |
| 音效 | 原版 `.SND`(FM Towns) |
...
#### ★ 第二種形態:**安靜截斷**(2026-08-08 踩到,比上面那種陰險)

`sub_CD28`(許願井)被反編譯成一段**看起來完整、邏輯自洽、還會 return 的函式** ——
沒有任何警告註解 —— 而組語版有 **三個參數、四倍長**,後面整個彩蛋都不見了:

```c
char sub_CD28()                    // ← ★ 參數列是空的
{
  印 "a well.  Drop a coin?";
  do 等鍵; while (不是 Y 也不是 N);
  if (N) return 印 "No";
  else   return 印 "Yes";          // ← 真正的內容從這裡才開始
}
```

代價:`strings.go` 掛了一句錯結論(「原版不管答什麼都沒有後續,組語裡沒有任何寫入」),
而**整個許願井機制被判定為不存在**。詳見 `docs/re/65`。

⇒ **判斷法:反編譯出來的函式參數列是空的(或明顯比呼叫端推的參數少),
就是被截斷了。** 一律回去讀組語 —— 尤其在下「原版沒有這件事」這種**否定結論**之前
(併 `~/diagnosis-notes/docs/02-query-returned-empty/`:查詢回空要先做正對照)。
...
- **★★ 讀函式一律讀到 `endp`。「看起來收尾了」不是停下來的理由。**
  原版把新東西掛在 `retn` 前面一行,而那一行在畫面上長得像收尾:

  | 位置 | 藏了什麼 |
  |---|---|
  | `sub_2D38` 最後一行 | **怪物移動**(`docs/re/85`)|
  | `sub_2A50C` 最後一行 | **再生戒指**(`docs/re/99`),而這支函式**已經逐行讀過一次** |

  ⇒ 同一個形狀踩了兩次,所以它是規則不是軼事。**已經讀過**的函式不算讀完 ——
  重讀時從 `proc` 讀到 `endp`,不要從上次停下的地方接。

- **★ 反編譯出來的常數參數(`f(0)`、`f(1)`)要回組語核一次。**
  Hex-Rays 會把全域**常數傳播**成字面值:`sub_2ECE8(0)` 的真身是
  `sub_2ECE8(byte_3E0AE)`,而那個全域是「當前行動的單位」。
  照反編譯寫會得到一條**捏造的規則**(「只有第一槽有效」),
  而它看起來完全合理 —— 這種錯誤不會有任何症狀。
  ⇒ 與 §4.4 的三種形態並列:**空參數列 = 截斷、常數回傳 = 摺疊、常數參數 = 傳播**。
...
| **P6** | 音樂與美術主題。音樂三來源:**FM Towns 兩條 CDDA**(直接轉 ogg,最省)、**15 首 `.EUP`**(EUPHONY,u1-cht 逆過可複用)、**upgrade 的 16 首 `.XMI`**(→MID→Roland MT-32,`tools/mt32_render.sh`;曲號對應靠旋律比對,`docs/re/98`);場景對應**要逆向**(`docs/re/87`;`.TBL` 只給曲號→檔名與音量)。美術主題:EGA / CGA / Hercules / **FM Towns 直色 `.TIF`** / PC-98 | 熱鍵切換不掉 NPC、不崩(u1-cht 踩過);顏色對得上各版實機截圖(⚠ TIF FillOrder=2、`.SND` 是 sign-magnitude PCM);音樂隨場景切換 |
