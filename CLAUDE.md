...
| `STORY.DAT` `QUESTION.DAT` `KARMA.DAT` `MISCMSG.DAT` `ENDMSG.DAT` `SHOPPE.DAT` `LOOK2.DAT` | **明文 ASCII**,`\|` 分隔記錄、`{` 起始標記、**`_` 是斷字提示**(`be_gin`、`mys_te_ri_ous`) | 開場故事 / 吉普賽問答 / 業報 / 系統訊息 / 商店 / 觀察文字。⚠ 譯成中文時 `_` 要移除,不可原樣保留 |
