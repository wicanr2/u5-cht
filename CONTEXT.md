...
- **GUI 方案 A（2026-08-10，已定案）**：現代版面為預設；以 `F2` 切換
  `UIOriginal` 原版外框／時間月相帶。現代版面同樣必須保留日／月圖（moon gate）指示；
  兩種模式都保留 remake 的繁中擴充狀態與訊息內容／捲動，不退回原版窄英文欄位。
  宣傳影片必須安排一段示範 `F2` 在兩種 UI 間切換。
...
| `docs/re/86` 沒有收進函式索引 | 一直都在裡面 —— 是 `check_doc_claims.py` 的 regex 只收小寫檔名,而那份檔名有 `0xDC`。**同一行 regex 第二次出錯**:`\d\d-` 收不到 `100-…`(讀到 `10` 之後要求 `-` 卻是 `0`)⇒ 編號改 `\d{2,}` | 閘門的**假警報**:每次都亮的那一行會讓人連真的那幾行一起跳過。⇒ 閘門報「缺東西」時,先確認**它找得到**再去補 | `tools/check_doc_claims.py` |
...
| `byte_3E0E0[i] & 0x80` 設著 = 地牢已封印 | **極性相反**:設著是**通的**,清掉(== 0)才是崩塌 | 拿常數的名字當語意,沒回去讀 `sub_1056C` 的 `setz`。四條獨立證據:`setz` / `INIT.GAM` 全 0 / `byte_55E18` 的三種地形 / look 文字「崩塌的地牢入口」 | `docs/re/99` §5e |
| 開局八座地牢入口都是通的 | **全是崩塌的** —— 地形由 `sub_105E4` 在載入地圖時依旗標重建,不是直接用 `BRIT.DAT` | 只看了「喊力量之言會改地形」,沒問「那開局的地形是誰決定的」。而 `SpeakWord` 的兩句話也因此是反的(第一次喊對會說「被封住了」)| `docs/re/99` §5e |
| 崩塌的入口擋不住玩家(`sub_2D564` 沒查封印)| 擋它的是**呼叫端的分派定義域**:`sub_2D72C` 是 `switch (tile − 0x10)`,而 `0xDF − 0x10 = 0xCF > 0x2E` ⇒ 落到 default 印「What?」| 只讀了「做事的那一支」,沒問「誰決定要不要呼叫它」。⇒ 引擎當時用**座標**查地牢,所以地形崩了人還是走得進去 | `docs/re/100` §1 |
| `TileStairsDown` = 0xC8 / `TileStairsUp` = 0xC9 | 反了:**0xC8 是上、0xC9 是下**(`sceneset.go` 的 `LadderUp`/`LadderDown` 才對)| 同一組值在兩個檔案裡各取了一次名字,而**兩邊沒有互相驗過**。判準有兩個獨立來源:`ClimbDelta(0xC8)` 回 +1、`sub_92C0` 要往上時找 0xC8 | `docs/re/100` §3 |
| 在地牢裡可以存檔 | 存不了 —— `validate` 的上限用 `len(Locations)`(32),而地牢的地點碼是 **0x21..0x28** | 拿「表的長度」當「值的上限」,而地點表其實有 40 筆(後 8 筆就是地牢)。⇒ 這是一個真的、玩家會遇到的 bug | `docs/re/100` §2 |
| 站在黑暗格上視野歸零 ⇒ 直接判 tile 0xFF 就好 | 要先確認**在大地圖上**:`TileBlank` 也是 0xFF,而 `TileAt` 在場景資料缺失時就回它 | 一個值有兩個意思,而「全黑」的症狀看起來像光照公式壞了 —— 不像「這格沒有資料」| `docs/re/100` §4 |
| 色號 6 是暗黃(0xAAAA00),因為 `DATA.OVL` 0x52EE 的表寫 `0x06` | 表確實寫 `0x06`,但**畫面上是棕 #AA5500**(對 DOSBox 原版取色,`machine=ega` 與 `vgaonly` 都是)⇒ 那張表沒活到遊戲畫面 | **把「驅動程式載入什麼」當成「畫面生效什麼」** —— 中間還有一段執行期,只有 reference 看得到。而且錯得很安靜:七成七的木頭色只是「看起來有點黃」| `docs/playtest-findings.md` §1、`docs/re/62` |
| 移動不必印任何訊息(只有撞牆才印)| **每走一步都印方向名**;三支函式各自印(`sub_86C` 場景 / `sub_2D174` 大地圖 / `sub_15F18` 戰鬥),只有揚帆的船不印 | 少一句回饋**不會讓任何測試變紅**,也不會讓程式壞掉 —— 只有把畫面並排才看得出來 | `docs/playtest-findings.md` §2 |
| 隊伍在地圖中央畫白框,Avatar 的 tile 索引「要從反編譯碼確認,不猜」| 索引早就在手上:**載具欄位本身就是 tile 碼**,`NPCTileBase + Transport`(地牢那條路一直這樣畫)| 一個 `TODO(不猜)` 擺著,而答案在**同一個檔案的另一個函式**裡 | `docs/playtest-findings.md` §3 |
| 兩個獨立的 tester 都走不出尤洛小屋 ⇒ 引擎的門壞了 | 門要先 `O)pen`;引擎的 `openToward` 一直都在,缺的是**驅動腳本沒有這個鍵** | 「兩邊都卡住」看起來像強證據,但它們共用同一份對照點清單 —— **清單漏一個鍵,兩邊一起漏** | `docs/playtest-findings.md` §5 |
