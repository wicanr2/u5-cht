...
| DOS + Exodus MIDI Upgrade(2001) | 同上 `upgrade/` | **16 首 `.XMI` 音樂**(+ 一個五首的測試檔 `setm`);8 個被 patch 的檔可當 overlay 結構 oracle。**沒有 VGA 美術** |
...
| upgrade 的 `.XMI` 有 **19 首** | 先更正成 15,再更正成 **16 首音樂 + 1 個五首的測試檔** | 19 是數了檔名清單裡的詞;15 是用 `*.XMI` 這種**大小寫敏感**的 glob 數的,漏掉小寫的 `trntlla.xmi`。⚠ 漏掉之後「15 首」看起來完全自洽(剛好等於 FM Towns 的 15 首)⇒ 錯了也沒有跡象 | `docs/formats/13` §0 |
| XMI 的時基固定 1/120 秒 | 由曲子自帶的 `FF 51` 決定(`U5THEME` 是 10 ms/tick)| 拿**沒對照過渲染結果**的估算式當驗過。`U5THEME` 算 123.5 秒、實際 148.2 秒(差 21%),而轉檔一直都是對的 —— 錯的只有估算 | `docs/formats/13` §2③ |
| MT-32 那批曲子沒辦法對上曲號(對應在 patch 過的 `ULTIMA.EXE` 裡)| **靠旋律配對解出 15/15**:音高差分的最長共同子串,雙向一致 14 組,平手那一組用音數與聲道數收尾 | 只想到「去逆那支 EXE」與「照曲名猜場景」兩條路,漏掉「兩套是同一批曲子 ⇒ 可以直接比內容」 | `docs/re/98` §3 |
...
| 多人可選時挑「最後一位能動的隊員」 | 原版**開選單問玩家**(`sub_2A7F4`),而且游標可以停在死人身上、按下去才說「無法行事!」並重問 | 把「原版在只有一人能動時回掃到的那一位」這個正確觀察,擴大成「多人時也是這樣」 | `docs/re/98` |
| `docs/re/86` 沒有收進函式索引 | 一直都在裡面 —— 是 `check_doc_claims.py` 的 regex 只收小寫檔名,而那份檔名有 `0xDC` | 閘門的**假警報**:每次都亮的那一行會讓人連真的那幾行一起跳過 | `tools/check_doc_claims.py` |
| 斷糧是**每回合**掉一次血 | **每小時**一次 —— 挨餓那條路的結尾 `jmp loc_2A5C0` 就是「記下這個小時處理過了」 | 只看到「用餐那條路才設 `byte_3E090`」,沒注意另外兩條路也 `jmp` 到同一點。⇒ 差別很具體:引擎讓斷糧在幾十步內滅團 | `docs/re/99` §5a |
| `byte_3E09B` 是**進餐**計數器 | **回合**計數器(每回合 +1,上限 255);歸零時機是施捨給乞丐 | 它的 `+1` 在「這小時已結算」那條路也會到的位置,而當時只看了用餐那條路。⇒ 連帶擋住乞丐業報一整個機制 | `docs/re/99` §5b |
| 再生戒指沒有效果(只會消失) | 每回合 1/8 回 1 點,上限 MaxHP | `sub_2BCC8` 被 Hex-Rays 反編譯成 `return 0;`(§4.4 第一種形態),而 `call` 它的那一行在 `sub_2A50C` 的 `retn` 前面一行(同 `docs/re/85` 的形狀,**第二次**) | `docs/re/99` §1 |
| `sub_2ECE8` 的參數是 0 ⇒ 只有第一槽的戒指有用 | 參數是 `byte_3E0AE`(**當前行動的單位**) | Hex-Rays 把一個全域常數傳播成字面 `0`;照它寫會多出一條捏造的規則 | `docs/re/99` §2 |
